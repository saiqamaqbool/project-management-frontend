...
const HRDashboard = () => {
  ...
  const { paginatedEmployees, totalEmployeePages } = useMemo(() => {
    ...
    const filtered = mergedEmployees.filter((emp) => {
      ...
      return fullName.includes(term) || emp.email?.toLowerCase().includes(term) || String(emp.employeeId).includes(term);
    });
  ...
  const PaginationControls = ({ currentPage, totalPages, setCurrentPage }) => (
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", marginTop: "15px" }}>
      <button onClick={() => setCurrentPage(Math.max(1, currentPage - 1))} disabled={currentPage === 1} style={{ margin: "0 5px", padding: "8px 15px", border: "1px solid #6A0DAD", background: "#fff", color: "#6A0DAD", cursor: "pointer", borderRadius: "5px" }}>Previous</button>
      <span style={{ margin: "0 15px" }}>Page {currentPage} of {totalPages}</span>
      <button onClick={() => setCurrentPage(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages || totalPages === 0} style={{ margin: "0 5px", padding: "8px 15px", border: "1px solid #6A0DAD", background: "#fff", color: "#6A0DAD", cursor: "pointer", borderRadius: "5px" }}>Next</button>
    </div>
  );
    ...
    <div>
      <div style={{ marginBottom: "15px", display: "flex", justifyContent: "flex-end" }}>
        <input
          ...
          onChange={(e) => { setEmployeeSearchTerm(e.target.value); setCurrentEmployeePage(1); }}
      ...
      <PaginationControls currentPage={currentEmployeePage} totalPages={totalEmployeePages} setCurrentPage={setCurrentEmployeePage} />
    </div>
  );
    ...
    <div style={{ marginTop: "40px" }}>
      ...
      <div style={{ marginBottom: "15px", display: "flex", justifyContent: "flex-end" }}>
        <input
          ...
          onChange={(e) => { setLeavesSearchTerm(e.target.value); setCurrentLeavesPage(1); }}
          style={{ padding: "10px", width: "300px", borderRadius: "5px", border: "1px solid #ccc" }}
        />
      </div>
      ...
      <PaginationControls currentPage={currentLeavesPage} totalPages={totalLeavesPages} setCurrentPage={setCurrentLeavesPage} />
    </div>
  );
        ...
        <div style={{ padding: "20px" }}>
          ...
          {!isFormVisible && (
            <>
              ...
              {empLoading ? <p>Loading employee data...</p> : <EmployeeTable />}
